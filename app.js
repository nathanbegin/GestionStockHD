...
function dedupeNamedCollection(collection, fallbackNames) {
  ...
  for (const raw of source) {
    ...
    if (existing) {
      ...
      if (new Date(raw?.updatedAt || 0) > new Date(existing.updatedAt || 0)) existing.updatedAt = raw.updatedAt;
      continue;
    }
    const entry = { id: raw?.id || crypto.randomUUID(), name, updatedAt: raw?.updatedAt || nowIso() };
...
function sanitizeState(raw) {
  ...
  const listsResult = dedupeNamedCollection(source.lists, ["Tournée principale"]);
  const departmentsResult = dedupeNamedCollection(source.departments, DEFAULT_DEPARTMENTS);
  ...
  for (const rawItem of Array.isArray(source.items) ? source.items : []) {
    if (!rawItem?.id || seenItems.has(rawItem.id)) continue;
  ...
  return {
    ...
    items,
    deletedIds: [...new Set(Array.isArray(source.deletedIds) ? source.deletedIds : [])],
    settings: {
  ...
  return `
    ...
    <section class="section">
      ...
      <div class="grid actions-grid">
        <button class="card action-card" data-action="go" data-view="scan"><span class="icon">▣</span><h3>Photographier ou téléverser</h3><p>Lire une étiquette avec la caméra ou une image existante.</p></button>
        ...
        <button class="card action-card" data-action="go" data-view="lists"><span class="icon">☷</span><h3>Voir les listes</h3><p>Filtrer, sélectionner et modifier plusieurs articles.</p></button>
...
function renderCompactItem(item) {
  return `<article class="card item-card"><div class="item-top"><div class="item-title"><div class="item-qty">${Number(item.quantity) || 1}</div><div><h3>${escapeHTML(item.name || "Article sans description")}</h3><p class="sku">${escapeHTML(formatSku(item.sku))}</p><p class="small muted">${escapeHTML(departmentName(item.departmentId))} · ${escapeHTML(listName(item.listId))}</p></div></div><button class="status-button" data-action="cycle-status" data-id="${item.id}" data-status="${item.status}">${STATUS_LABELS[item.status]}</button></div></article>`;
}
  ...
  return `<section class="section"><div class="section-head"><div><h2>Lire une étiquette</h2><p class="muted">Prends une nouvelle photo ou sélectionne une image déjà enregistrée.</p></div></div>
    ...
    <section class="section"><div class="section-head"><div><h2>Résultat à confirmer</h2><p class="muted">Le numéro est conservé sous la forme 1001 123 456.</p></div></div>
      <form id="scanForm" class="card"><div class="form-grid">
        <label>Numéro d’article / SKU${skuField(scanDraft.sku)}</label>
...
function filteredItems() {
  ...
  return state.items.filter(item => {
    const textMatch = [item.sku, item.name, item.salesLocation, item.stockLocation, item.note].some(v => String(v || "").toLowerCase().includes(q));
...
function renderBulkToolbar(items) {
  ...
  return `<div class="selection-toolbar card">
    ...
    ${selectedIds.size ? `<div class="bulk-actions">
      ...
      <button class="button compact" data-action="clear-selection">Annuler la sélection</button>
    </div>` : ""}
  </div>`;
}
...
function renderLists() {
  ...
  return `<section class="section"><div class="section-head"><div><h2>${items.length} article${items.length > 1 ? "s" : ""}</h2><p class="muted">Sélectionne plusieurs articles pour modifier leur statut, leurs champs ou les supprimer.</p></div><button class="button primary" data-action="go" data-view="tour">Mode remplissage</button></div>
    <div class="toolbar">
...
function renderBulkEditDialog() {
  return `<div class="dialog-backdrop" role="presentation">
    <div class="dialog" role="dialog" aria-modal="true" aria-labelledby="bulkEditTitle">
      ...
      <form id="bulkEditForm">
        ...
        <div class="bulk-field"><label class="check-label"><input type="checkbox" name="applySalesLocation"> Modifier l’emplacement tablette</label><input name="salesLocation" maxlength="120" placeholder="Vide = effacer l’emplacement"></div>
        <div class="bulk-field"><label class="check-label"><input type="checkbox" name="applyStockLocation"> Modifier l’emplacement de réserve</label><input name="stockLocation" maxlength="120" placeholder="Vide = effacer l’emplacement"></div>
        <div class="bulk-field"><label class="check-label"><input type="checkbox" name="applyNote"> Modifier la note</label><textarea name="note" maxlength="500" placeholder="Vide = effacer la note"></textarea></div>
        <div class="form-actions"><button class="button primary" type="submit">Appliquer aux articles sélectionnés</button><button class="button" type="button" data-action="bulk-edit-close">Annuler</button></div>
      </form>
    </div>
  </div>`;
}
...
function renderSettings() {
  return `<section class="section"><div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(290px,1fr))">
    ...
    <article class="card"><h2>Départements</h2><p class="muted small">Les doublons de nom sont supprimés automatiquement lors du chargement et de la synchronisation.</p><form id="departmentForm" class="inline-form"><label>Nouveau département<input name="name" required maxlength="60" placeholder="Ex. Saisonnier"></label><button class="button primary" type="submit">Ajouter</button></form><div class="manage-list">${state.departments.map(x => `<div class="manage-row"><span>${escapeHTML(x.name)}</span><button class="button compact danger" data-action="delete-department" data-id="${x.id}">Supprimer</button></div>`).join("")}</div></article>
...
function addNamedEntry(form) {
  ...
  if (target.some(entry => normalizeName(entry.name) === normalizeName(name))) {
    toast("Cet élément existe déjà");
    return;
  }
...
els.appMain.addEventListener("submit", e => {
  ...
  if (form.id === "itemForm") {
    ...
    const item = formToItem(form, existing);
    if (!item || !upsertItem(item)) return;
  ...
  if (form.id === "listForm" || form.id === "departmentForm") addNamedEntry(form);
  ...
  if (form.id === "bulkEditForm") applyBulkEdit(form);
});

els.appMain.addEventListener("change", async e => {
  if (["cameraInput", "galleryInput"].includes(e.target.id) && e.target.files?.[0]) await handlePhoto(e.target.files[0]);
...
els.appMain.addEventListener("click", async e => {
  ...
  if (action === "edit-item") { editingId = btn.dataset.id; currentView = "manual"; selectedIds.clear(); render(); }
  ...
  if (action === "delete-item") {
    ...
    deleteItems([btn.dataset.id]); render(); toast("Article supprimé");
  }
  if (action === "bulk-status") {
    ...
    render(); toast(`Statut appliqué à ${selected.length} article${selected.length > 1 ? "s" : ""}`);
  }
  if (action === "bulk-delete") {
    if (!selectedIds.size || !confirm(`Supprimer les ${selectedIds.size} articles sélectionnés?`)) return;
    const count = selectedIds.size;
    deleteItems([...selectedIds]); render(); toast(`${count} article${count > 1 ? "s supprimés" : " supprimé"}`);
  }
  if (action === "bulk-edit-open") { bulkEditOpen = true; render(); }
  if (action === "bulk-edit-close") { bulkEditOpen = false; render(); }
  if (action === "clear-selection") { selectedIds.clear(); bulkEditOpen = false; render(); }
  ...
  if (action === "clear-photo") { scanDraft = emptyScanDraft(); render(); }
...
async function analyzePhoto(btn) {
  ...
  try {
    const response = await fetch("/api/analyze", {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-app-pin": session.pin },
      body: JSON.stringify({ image: scanDraft.photo })
    });
...
async function syncNow() {
  ...
  try {
    ...
    updateSyncIndicator("error"); toast(error.message);
  }
}
...
function exportCSV() {
  ...
  const rows = state.items.map(x => [formatSku(x.sku), x.name, listName(x.listId), departmentName(x.departmentId), x.quantity, PRIORITY_LABELS[x.priority], STATUS_LABELS[x.status], x.salesLocation, x.stockLocation, x.note, x.updatedBy, x.updatedAt]);
...
els.importInput.addEventListener("change", async e => {
  ...
  try {
    ...
    saveState(); render(); toast("Sauvegarde importée et doublons nettoyés");
  } catch (error) {
    toast(error.message);
  }
  e.target.value = "";
