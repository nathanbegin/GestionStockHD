...
async function removeExpiredSubscription(supabase, rowId) {
  try {
    const { error } = await supabase.from("app_state").delete().eq("id", rowId);
    if (error) console.warn("Nettoyage d’un abonnement Web Push expiré", error.message);
  } catch (error) {
    console.warn("Nettoyage d’un abonnement Web Push expiré", error?.message || error);
  }
}
    ...
    for (const entry of subscriptions) {
      try {
        ...
        if ([404, 410].includes(statusCode)) {
          await removeExpiredSubscription(supabase, entry.rowId);
        } else {
          failed += 1;
