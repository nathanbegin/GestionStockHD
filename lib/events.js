...
async function mutateParticipation(supabase, profile, user, eventId, join) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    if (join) {
      ...
      if (updated.createdByUserId && updated.createdByUserId !== String(user.id)) {
        await notifyUsers(
          supabase,
          ...
          updated,
          actor,
          "Nouvelle inscription à ton événement",
          `${actor.name} s’est inscrit à « ${updated.title} » le ${eventDateLabel(updated)}.`
        );
      }
    }
