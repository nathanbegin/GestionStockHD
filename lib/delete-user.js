...
async function cleanupEventParticipants(supabase, userId) {
  const { data, error } = await supabase
    .from("app_state")
    .select("id,snapshot")
  ...
  const results = await Promise.all(rows.map(row =>
    supabase.from("app_state").update({
      snapshot: {
        ...(row.snapshot || {}),
        participant_ids: row.snapshot.participant_ids.filter(id => id !== userId),
        updated_at: now
      },
      updated_at: now
    }).eq("id", row.id)
...
export async function deleteUserAccount(supabase, {
  ...
  const cleanupResults = await Promise.allSettled([
    ...
    cleanupEventParticipants(supabase, userId),
