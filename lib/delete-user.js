...
async function cleanupEventParticipation(supabase, userId) {
  const { data, error } = await supabase
    .from("app_state")
    .select("id,snapshot")
  ...
  const affected = (data || []).filter(row =>
    Array.isArray(row?.snapshot?.participant_ids) && row.snapshot.participant_ids.includes(userId)
  );
  if (!affected.length) return;

  const now = new Date().toISOString();
  const results = await Promise.all(affected.map(row =>
    supabase.from("app_state").update({
      snapshot: {
        ...(row.snapshot || {}),
        participant_ids: [...new Set((row.snapshot.participant_ids || []).filter(id => id !== userId))],
        updated_at: now
      },
      updated_at: now
    }).eq("id", row.id)
...
export async function deleteUserAccount(supabase, {
  ...
  const cleanupResults = await Promise.allSettled([
    ...
    cleanupEventParticipation(supabase, userId),
