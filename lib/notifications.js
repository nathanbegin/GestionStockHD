...
async function existingNotificationIds(supabase, ids) {
  const existing = new Set();
  for (let index = 0; index < ids.length; index += 100) {
    const chunk = ids.slice(index, index + 100);
    const { data, error } = await supabase.from("app_state").select("id").in("id", chunk);
...
export async function createAssignmentNotifications(supabase, {
  ...
  if (!before || !after) return { created: 0, push: { sent: 0, failed: 0, subscriptions: 0 } };

  const approvedIds = new Set((approvedUserIds || []).map(String));
  if (!approvedIds.size) return { created: 0, push: { sent: 0, failed: 0, subscriptions: 0 } };
  ...
  if (!rows.length) return { created: 0, push: { sent: 0, failed: 0, subscriptions: 0 } };
  const existing = await existingNotificationIds(supabase, rows.map(row => row.id));
  const newRows = rows.filter(row => !existing.has(row.id));
  if (!newRows.length) return { created: 0, push: { sent: 0, failed: 0, subscriptions: 0 } };

  const { error } = await supabase.from("app_state").insert(newRows);
  if (error) {
    if (String(error.code || "") === "23505") {
      const { error: retryError } = await supabase.from("app_state").upsert(newRows, {
        onConflict: "id",
        ignoreDuplicates: true
      });
      if (retryError) throw retryError;
      return { created: 0, push: { sent: 0, failed: 0, subscriptions: 0 } };
    }
    throw error;
  }
  ...
  try {
    push = await sendWebPushNotifications(supabase, newRows);
  } catch (error) {
    console.warn("Notifications Web Push indisponibles", error?.message || error);
  }
  return { created: newRows.length, push };
}
